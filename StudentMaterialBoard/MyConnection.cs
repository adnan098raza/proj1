...
namespace StudentMaterialBoard
{
    public class MyConnection
    {
        ...
        public MyConnection()
        {
            con = new MySqlConnection("server=localhost;database=materialboard;user id=root;password=root;port=3306;");
            con.Open();
        }
        ...
        public string StudentRegister(string USN,string DeptId, string Name, string Password, string EmailId)
        {
            cmd = new MySqlCommand();
            cmd.Connection = con;
            string result = "";
            string sql = string.Format("insert into studentmaster(SUSN,DeptId,StudentName,Password,EmailId,MUCount,MRCount,Status)values('{0}',{1},'{2}','{3}','{4}',0,0,'Active')", USN,int.Parse(DeptId), Name, Password, EmailId);
            cmd.CommandText = sql;
            result = cmd.ExecuteNonQuery().ToString();
            con.Close();
            return result;
...
//                                        materialmaster.SpamCount,materialmaster.Description");
        ...
        public DataTable GetLeaderBoard()
        {
            cmd = new MySqlCommand();
            cmd.Connection = con;
            string sql = string.Format("SELECT SUSN,StudentName,((MUCount*10)+(MRCount*2)) AS Score FROM studentmaster where ((MUCount*10)+(MRCount*2)) > 0 ORDER BY Score DESC LIMIT 5");
            cmd.CommandText = sql;
            DataTable tab = new DataTable();
            adp = new MySqlDataAdapter(cmd);
            adp.Fill(tab);
        ...
        public DataTable GetMaterial_DeptId(string DAId)
        {
            cmd = new MySqlCommand();
            cmd.Connection = con;
            string sql = string.Format("select * from deptadmin where DAId='{0}'", DAId);
            ...
            string sqllp = string.Format("select materialmaster.MMId,materialmaster.MaterialName,materialmaster.UploadDate from materialmaster inner join studentmaster on materialmaster.UId=studentmaster.SUSN where studentmaster.DeptId={0} and materialmaster.Status='Active'", int.Parse(tab.Rows[0]["DeptId"].ToString()));
            cmd.CommandText = sqllp;
            DataTable tablp = new DataTable();
            adp = new MySqlDataAdapter(cmd);
            adp.Fill(tablp);
        ...
        public string MaterialDeactive(int MMId)
        {
        ...
        public DataTable GetStudentEmail(string StudentId)
        {
            cmd = new MySqlCommand();
            cmd.Connection = con;
            string sql = string.Format("select * from studentmaster where SUSN='{0}'", StudentId);
            cmd.CommandText = sql;
            DataTable tab = new DataTable();
            adp = new MySqlDataAdapter(cmd);
            adp.Fill(tab);

            string sqlem = string.Format("select EmailId from studentmaster where DeptId={0} and SUSN<>'{1}'", int.Parse(tab.Rows[0]["DeptId"].ToString()), StudentId);
            cmd.CommandText = sqlem;
            DataTable tabem = new DataTable();
            adp = new MySqlDataAdapter(cmd);
            adp.Fill(tabem);
            con.Close();
            return tabem;
        }
        public DataTable GetLecturerEmail(string LecturerId)
        {
            cmd = new MySqlCommand();
            cmd.Connection = con;
            string sql = string.Format("select * from lecturemaster where EmailId='{0}'", LecturerId);
            cmd.CommandText = sql;
            DataTable tab = new DataTable();
            adp = new MySqlDataAdapter(cmd);
            adp.Fill(tab);

            string sqlem = string.Format("select EmailId from studentmaster where DeptId={0}", int.Parse(tab.Rows[0]["DeptId"].ToString()));
            cmd.CommandText = sqlem;
            DataTable tabem = new DataTable();
            adp = new MySqlDataAdapter(cmd);
            adp.Fill(tabem);
        ...
        public DataTable GetMaterial_MId(int MMId)
        {
            cmd = new MySqlCommand();
            cmd.Connection = con;
            string sql = string.Format("select * from materialmaster where MMId={0}", MMId);
            cmd.CommandText = sql;
            DataTable tab = new DataTable();
            adp = new MySqlDataAdapter(cmd);
            adp.Fill(tab);
        ...
        public DataTable GetMaterialQuery_MQId(int MQId)
        {
            cmd = new MySqlCommand();
            cmd.Connection = con;
            string sql = string.Format("select * from materialquery where MQId={0}", MQId);
            cmd.CommandText = sql;
            DataTable tab = new DataTable();
            adp = new MySqlDataAdapter(cmd);
            adp.Fill(tab);
