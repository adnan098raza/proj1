...
{
    public partial class MaterialQuery : System.Web.UI.Page
    {
        static string MMId;
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (res == "1")
            {
                ...
                tab = obj.GetStudentEmail(Session["UserId"].ToString());
                obj = new MyConnection();
                DataTable tabmm = new DataTable();
                tabmm = obj.GetMaterial_MId(int.Parse(MMId));
                if (tab.Rows.Count > 0)
                {
                    for (int i = 0; i < tab.Rows.Count; i++)
                    {
                        SendEmail.Send(tab.Rows[i]["EmailId"].ToString(), tabmm.Rows[0]["MaterialName"].ToString() + " - Material New Query Posted","Material New Query Posted");
                    }
                }
                txtQuery.Text = "";
        ...
        protected void btnPostReply_Click(object sender, EventArgs e)
        {
            ...
            if (res == "1")
            {
                ...
                obj = new MyConnection();
                DataTable tabmm = new DataTable();
                tabmm = obj.GetMaterialQuery_MQId(int.Parse(Session["MQId"].ToString()));
                if (tab.Rows.Count > 0)
                {
                    for (int i = 0; i < tab.Rows.Count; i++)
                    {
                        SendEmail.Send(tab.Rows[i]["EmailId"].ToString(), tabmm.Rows[0]["PostQuery"].ToString() + " - Material Posted Query, New Reply Posted", "Material Posted Query, New Reply Posted");
                    }
                }
                ClientScript.RegisterStartupScript(this.GetType(), "Popup", "ShowMsg();", true);
